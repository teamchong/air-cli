...
# air-cli Installer - Agentic Information Retrieval
set -e
...
echo "🌬️  air-cli Installer"
echo "==========================="
echo "Agentic Information Retrieval"
echo ""
...
if [ -f "$CLAUDE_MD" ]; then
    ...
    echo "<!-- BEGIN AIR-CLI -->" >> "$CLAUDE_MD.tmp"
    ...
    if [ -f "$SCRIPT_DIR/CLAUDE_INSTRUCTIONS.md" ]; then
        ...
        echo "## air-cli - Agentic Information Retrieval" >> "$CLAUDE_MD.tmp"
        echo "Browser automation and content curation tool. Run 'air --help' for documentation." >> "$CLAUDE_MD.tmp"
    fi

    echo "<!-- END AIR-CLI -->" >> "$CLAUDE_MD.tmp"

    mv "$CLAUDE_MD.tmp" "$CLAUDE_MD"
    echo "✅ Updated CLAUDE.md with air-cli instructions"
else
    # New file - create with section from CLAUDE_INSTRUCTIONS.md
    mkdir -p "$CLAUDE_DIR"

    echo "<!-- BEGIN AIR-CLI -->" > "$CLAUDE_MD"
    ...
    if [ -f "$SCRIPT_DIR/CLAUDE_INSTRUCTIONS.md" ]; then
        ...
        # Fallback minimal content if file not found
        echo "## air-cli - Agentic Information Retrieval" >> "$CLAUDE_MD"
        echo "Browser automation and content curation tool. Run 'air --help' for documentation." >> "$CLAUDE_MD"
    fi

    echo "<!-- END AIR-CLI -->" >> "$CLAUDE_MD"

    echo "✅ Created CLAUDE.md with air-cli instructions"
fi
...
echo ""
echo "Run 'air --help' to see all available commands"
echo "Agentic Information Retrieval - Take back control of your information diet"