...
describe('DIContainer', () => {
  ...
  describe('basic functionality', () => {
    it('should register and resolve services', () => {
      ...
      const resolved = testContainer.resolve<IBrowserService>(
    ...
    it('should cache factory results', () => {
      ...
      const resolved1 = testContainer.resolve<IBrowserService>(
        SERVICE_TYPES.BrowserService
      );
      const resolved2 = testContainer.resolve<IBrowserService>(
        SERVICE_TYPES.BrowserService
      );
    ...
    it('should throw error for unregistered services', () => {
      ...
      expect(() => testContainer.resolve(unknownToken)).toThrow(
        'Service not found'
      );
    });
...
describe('Global Container Setup Functions', () => {
  ...
  it('should setup default services in global container', () => {
    ...
    expect(() =>
      container.resolve<IBrowserService>(SERVICE_TYPES.BrowserService)
    ).not.toThrow();
  });
  ...
  it('should setup test services in global container', () => {
    ...
    const service = container.resolve<IBrowserService>(
      SERVICE_TYPES.BrowserService
    );
  ...
  it('should setup test services with custom mocks in global container', async () => {
    const mockBrowser = {
      ...
      // Add minimal Browser interface props to satisfy type checker
      removeAllListeners: () => mockBrowser,
      on: () => mockBrowser,
      once: () => mockBrowser,
...
describe('MockBrowserService', () => {
  ...
  it('should return mock values for utility methods', async () => {
    ...
    expect(await mockService.createTabHTTP(9222, 'https://test.com')).toBe(
      true
    );
