...
export class PerformanceMonitor {
  ...
  recordMetrics(metrics: PerformanceMetrics): void {
    ...
    if (metrics.executionTimeMs > this.thresholds.slowOperationMs) {
      logger.warn(
        `Slow operation detected: ${metrics.commandName} took ${metrics.executionTimeMs}ms`
      );
    }
    ...
    if (metrics.memoryUsageMB > this.thresholds.memoryWarningMB) {
      logger.warn(
        `High memory usage: ${metrics.commandName} used ${metrics.memoryUsageMB}MB`
      );
    }

    // Debug log all performance data
    logger.debug(
      `Performance: ${metrics.commandName} - ${metrics.executionTimeMs}ms, ${metrics.memoryUsageMB}MB, ${metrics.success ? 'success' : 'failed'}`
    );
  ...
  setThresholds(thresholds: Partial<PerformanceThresholds>): void {
    ...
    logger.debug(
      `Performance thresholds updated: ${JSON.stringify(this.thresholds)}`
    );
  }
