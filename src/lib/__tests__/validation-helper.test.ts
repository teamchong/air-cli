...
describe('ValidationHelper', () => {
  describe('validateCommandParams', () => {
    ...
    it('should handle invalid parameters', () => {
      ...
      expect(
        result.errors.some(e => e.includes('url must be a valid URL'))
      ).toBe(true)
      expect(
        result.errors.some(e => e.includes('port must be between 1 and 65535'))
      ).toBe(true)
      expect(
        result.errors.some(e => e.includes('timeout must be a positive number'))
      ).toBe(true)
    })
...
describe('ValidationSchemaBuilder', () => {
  ...
  it('should support custom validators in schema', () => {
    const schema = ValidationHelper.createSchema().custom('email', [
      (value, fieldName) => ({
    ...
    const invalidResult = schema.validate(invalidParams, {
      throwOnError: false,
    })
