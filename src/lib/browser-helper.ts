...
export class BrowserHelper {
  ...
  static async findPageById(port: number, tabId: string): Promise<Page | null> {
    ...
    if (!/^[0-9A-F]{32}$/i.test(tabId)) {
      console.debug(
        `Tab ID "${tabId}" doesn't match CDP format, skipping search`
      );
      return null;
    }
    ...
    return this.withBrowser(port, async browser => {
      ...
      try {
        ...
        console.debug(
          'Browser registry lookup failed, falling back to CDP scan:',
          error
        );
      }
   ...
   */
  private static async ensureTabHealthy(
    page: Page,
    tabId?: string
  ): Promise<void> {
      ...
      for (const context of browser.contexts()) {
        for (const page of context.pages()) {
          try {
            ...
            console.debug(
              `Could not clear browsing data for page ${page.url()}:`,
              error
            );
          }
        }
