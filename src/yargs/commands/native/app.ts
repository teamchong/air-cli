...
export const appCommand = createCommand<AppCommandOptions>({
  ...
  builder: yargs =>
    yargs
      ...
      .option('action', {
        type: 'string',
        describe: 'Pre-defined action (mail-inbox, mail-unread, calendar-today, calendar-all)',
      })
      .option('script', {
        type: 'string',
      ...
      .example('$0 app Mail --action mail-unread', 'Get unread emails from Mail.app')
      ...
      .example('$0 app Mail --script "Application(\'Mail\').inbox.messages().length"', 'Custom JXA'),
  ...
  handler: async context => {
    ...
    try {
      ...
      if (argv.tree) {
        ...
        if (!isRunning) {
          throw new Error(`${appName} is not running. Use --launch to start it.`)
        }
      ...
      if (argv.action) {
        await handleAction(argv.action, appName, argv.json, logger)
        return
      }
...
async function handleAction(
  ...
  switch (action) {
    ...
    default:
      throw new Error(`Unknown action: ${action}. Available: mail-inbox, mail-unread, calendar-today, calendar-all`)
  }
}
