...
describe('upload command - TAB ID FROM OUTPUT', () => {
  ...
  async function uploadFilesDirect(
    ...
    await BrowserHelper.withTargetPage(TEST_PORT, undefined, tabId, async page => {
      ...
      const backendNodeId = await page.evaluate((sel) => {
        ...
        return (el as any).__playwright__backendNodeId ||
  ...
  beforeAll(async () => {
    ...
    const { CDPConnectionPool } = await import('../../../../lib/cdp-connection-pool');
  ...
  describe('direct tab targeting with captured ID', () => {
    it('should upload single file using captured tab ID', async () => {
      ...
      await BrowserHelper.withTargetPage(TEST_PORT, undefined, testTabId, async page => {
    ...
    it('should upload multiple files', async () => {
      ...
      await BrowserHelper.withTargetPage(TEST_PORT, undefined, testTabId, async page => {
        const fileCount = await page.evaluate(() => {
          const input = document.querySelector('#multi-file') as HTMLInputElement;
    ...
    it('should work with different file inputs', async () => {
      ...
      await BrowserHelper.withTargetPage(TEST_PORT, undefined, testTabId, async page => {
        const bothHaveFiles = await page.evaluate(() => {
          const doc = document.querySelector('#doc-upload') as HTMLInputElement;
          const img = document.querySelector('#image-upload') as HTMLInputElement;
          return (doc?.files?.length || 0) > 0 && (img?.files?.length || 0) > 0;
        });
