...
describe('press command - TAB ID FROM OUTPUT', () => {
  ...
  describe('direct tab targeting with captured ID', () => {
    ...
    it('should press different keys in same tab', async () => {
      ...
      expect(
        runCommand(
          `${CLI} press Tab --tab-id ${testTabId} --port ${TEST_PORT}`,
          10000
        ).exitCode
      ).toBe(0)
      expect(
        runCommand(
          `${CLI} press Escape --tab-id ${testTabId} --port ${TEST_PORT}`,
          10000
        ).exitCode
      ).toBe(0)
      expect(
        runCommand(
          `${CLI} press Space --tab-id ${testTabId} --port ${TEST_PORT}`,
          10000
        ).exitCode
      ).toBe(0)
    }, 15000)

    it('should press arrow keys in specific tab', async () => {
      // Navigate to test page
      runCommand(
      ...
      expect(
        runCommand(
          `${CLI} press ArrowLeft --tab-id ${testTabId} --port ${TEST_PORT}`,
          10000
        ).exitCode
      ).toBe(0)
      expect(
        runCommand(
          `${CLI} press ArrowRight --tab-id ${testTabId} --port ${TEST_PORT}`,
          10000
        ).exitCode
      ).toBe(0)
      expect(
        runCommand(
          `${CLI} press ArrowUp --tab-id ${testTabId} --port ${TEST_PORT}`,
          10000
        ).exitCode
      ).toBe(0)
      expect(
        runCommand(
          `${CLI} press ArrowDown --tab-id ${testTabId} --port ${TEST_PORT}`,
          10000
        ).exitCode
      ).toBe(0)
    }, 20000)
