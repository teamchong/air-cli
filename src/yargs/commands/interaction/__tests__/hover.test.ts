...
describe('hover command - TAB ID FROM OUTPUT', () => {
  ...
  describe('direct tab targeting with captured ID', () => {
    ...
    it('should handle non-existent element gracefully', () => {
      ...
      expect(() => {
        runCommand(
          `${CLI} hover "#nonexistent" --tab-id ${testTabId} --port ${TEST_PORT}`,
          2000
        )
    ...
    it('should work with different element types', () => {
      ...
      expect(
        runCommand(
          `${CLI} hover "#hover-btn" --tab-id ${testTabId} --port ${TEST_PORT}`
        ).exitCode
      ).toBe(0)
      expect(
        runCommand(
          `${CLI} hover "#hover-span" --tab-id ${testTabId} --port ${TEST_PORT}`
        ).exitCode
      ).toBe(0)
    })
