...
describe('fill command - TAB ID FROM OUTPUT', () => {
  ...
  describe('direct tab targeting with captured ID', () => {
    ...
    it('should fill different input types in same tab', () => {
      ...
      expect(
        runCommand(
          `${CLI} fill "#text=Sample Text" --tab-id ${testTabId} --port ${TEST_PORT}`
        ).exitCode
      ).toBe(0)
      expect(
        runCommand(
          `${CLI} fill "#pass=secret123" --tab-id ${testTabId} --port ${TEST_PORT}`
        ).exitCode
      ).toBe(0)
      expect(
        runCommand(
          `${CLI} fill "#msg=Long message text" --tab-id ${testTabId} --port ${TEST_PORT}`
        ).exitCode
      ).toBe(0)
    })
