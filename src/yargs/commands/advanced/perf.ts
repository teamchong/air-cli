  ...
  handler: async cmdContext => {
    try {
      ...
      if (argv.slow) {
        ...
        logger.info(
          chalk.yellow(`🐌 Slow Operations (${slowOps.length} found):`)
        );
        logger.info('');
        ...
        slowOps.forEach((op, index) => {
          ...
          logger.info(
            `${index + 1}. ${status} ${op.commandName} - ${time} | ${memory}`
          );
      ...
      if (stats.totalCommands === 0) {
        ...
        logger.info(
          chalk.gray(
            'Run some commands to start collecting performance metrics'
          )
        );
      ...
      logger.info(
        `${chalk.cyan('Failure Rate:')} ${stats.failureRate.toFixed(1)}%`
      );
      logger.info('');
      ...
      if (stats.slowestCommand) {
        ...
        logger.info(
          `   ${stats.slowestCommand.commandName} - ${stats.slowestCommand.executionTimeMs}ms`
        );
        logger.info(
          `   ${stats.slowestCommand.memoryUsageMB}MB | ${stats.slowestCommand.timestamp.toISOString()}`
        );
        logger.info('');
      }
      ...
      if (stats.fastestCommand) {
        ...
        logger.info(
          `   ${stats.fastestCommand.commandName} - ${stats.fastestCommand.executionTimeMs}ms`
        );
        logger.info(
          `   ${stats.fastestCommand.memoryUsageMB}MB | ${stats.fastestCommand.timestamp.toISOString()}`
        );
        logger.info('');
      }
      ...
      if (recentSlowOps.length > 0) {
        ...
        recentSlowOps.forEach(op => {
          ...
          logger.info(
            `   ${status} ${op.commandName} - ${op.executionTimeMs}ms`
          );
        });
        logger.info('');
