...
describe('open command - REAL INTEGRATION TEST', () => {
  ...
  it('should launch browser and navigate to URL', async () => {
    const output = execSync(
      `${CLI_PATH} open https://example.com --port ${TEST_PORT}`,
      {
        encoding: 'utf8',
      }
    )
    ...
    const listOutput = execSync(`${CLI_PATH} list --port ${TEST_PORT}`, {
      encoding: 'utf8',
    })
  ...
  it('should connect to existing browser session', async () => {
    ...
    const output = execSync(
      `${CLI_PATH} open --new-tab https://google.com --port ${TEST_PORT}`,
      {
        encoding: 'utf8',
      }
    )
  ...
  it('should handle --new-tab flag', async () => {
    const output = execSync(
      `${CLI_PATH} open --new-tab https://github.com --port ${TEST_PORT}`,
      {
        encoding: 'utf8',
      }
    )
