...
export const connectMeshCommand = createCommand<ConnectMeshOptions>({
  metadata: { name: 'connect', category: 'mesh', description: 'Connect to another mesh node', aliases: [] },
  command: 'connect',
  ...
  handler: async (context) => {
    ...
    try {
      ...
      console.log('\n💡 Call services:');
      console.log(`   air mesh call --from ${localNode} --to ${remoteNode} --service <name>`);
    } catch (error: any) {
