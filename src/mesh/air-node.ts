...
export class AirNode {
   ...
   */
  async call(nodeName: string, serviceName: string, ...args: any[]): Promise<any> {
    ...
    const response = await this.p2pNode.request(nodeName, `service:${serviceName}`, {
      args
    });
  ...
  private startMDNSDiscovery(): void {
    try {
      ...
      this.bonjour.find({ type: 'air-node' }, (service: any) => {
        ...
        console.log(`🔍 Discovered node: ${service.name} at ${service.host}:${service.port}`);

        // Auto-connect to discovered nodes
        this.p2pNode
          .connectToPeer(service.name, service.host, service.port)
          .catch((err: Error) => {
            console.error(`❌ Failed to connect to ${service.name}:`, err.message);
          });
      });
    } catch (error: any) {
      console.warn(`⚠️  mDNS discovery failed: ${error.message}`);
      console.warn('💡 Local discovery disabled. Use relay for internet connectivity.');
    }
  }
  ...
  private async connectToRelay(): Promise<void> {
    ...
    console.log(`🌐 Relay connection not yet implemented: ${this.options.relay}`);
  }
