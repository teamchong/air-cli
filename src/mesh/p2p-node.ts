...
export type MessageHandler = (message: P2PMessage, peer: P2PPeer) => Promise<any>
...
export class P2PNode {
    ...
    this.wss.on('connection', (ws: WebSocket, req: any) => {
      ...
      ws.on('message', async (data: Buffer) => {
        try {
          ...
          console.error(`❌ Error handling message from ${peerName}:`, error.message);
        }
      });
   ...
   */
  async connectToPeer(peerName: string, host: string, port: number): Promise<void> {
    const paths = certificateManager.getNodePaths(this.nodeName);
   ...
   */
  private async handleMessage(message: P2PMessage, peer: P2PPeer): Promise<void> {
   ...
   */
  async request(peerName: string, messageType: string, data: any): Promise<any> {
