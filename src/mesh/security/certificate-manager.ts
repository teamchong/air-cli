...
export class CertificateManager {
  ...
  async generateNodeCertificate(nodeName: string): Promise<CertificateInfo> {
    ...
    try {
      ...
      throw new Error(`Failed to generate certificate for ${nodeName}: ${error.message}`);
    }
  }
  ...
  verifyCertificate(certPath: string): boolean {
    try {
      execSync(
        `openssl verify -CAfile "${this.caCertPath}" "${certPath}"`,
        { stdio: 'pipe' }
      );
      return true;
    } catch {
  ...
  hasNodeCertificate(nodeName: string): boolean {
    ...
    return existsSync(paths.key) && existsSync(paths.cert) && existsSync(paths.ca);
  }
}
